...
use karlsen_bip32::secp256k1::XOnlyPublicKey;
use karlsen_wallet_core::message::SignMessageOptions;
...
impl Message {
    ...
    async fn display_help(self: Arc<Self>, ctx: Arc<KarlsenCli>, _argv: Vec<String>) -> Result<()> {
        ctx.term().help(
            &[
                ...
                (
                    "verify <karlsen_address> <signature>",
                    "Verify the signature against the message and karlsen_address. Prompts for message.",
                ),
            ],
            None,
        )?;
    ...
    async fn sign(self: Arc<Self>, ctx: Arc<KarlsenCli>, karlsen_address: &str, message: &str) -> Result<()> {
        ...
        let privkey = self.get_address_private_key(&ctx, karlsen_address).await?;
