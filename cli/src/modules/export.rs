use crate::imports::*;
use karlsen_wallet_core::account::{multisig::MultiSig, Account, BIP32_ACCOUNT_KIND, MULTISIG_ACCOUNT_KIND};
...
async fn export_multisig_account(ctx: Arc<KarlsenCli>, account: Arc<MultiSig>) -> Result<()> {
    match &account.prv_key_data_ids() {
        ...
        Some(prv_key_data_ids) => {
            ...
            for (id, prv_key_data_id) in prv_key_data_ids.iter().enumerate() {
                ...
                let xpub_key: karlsen_bip32::ExtendedPublicKey<karlsen_bip32::secp256k1::PublicKey> =
