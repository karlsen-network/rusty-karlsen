...
use karlsen_wallet_core::account::{
    BIP32_ACCOUNT_KIND, LEGACY_ACCOUNT_KIND, MULTISIG_ACCOUNT_KIND,
};
use std::sync::Arc;
...
pub async fn prompt_for_mnemonic(term: &Arc<Terminal>) -> Result<Vec<String>> {
    ...
    loop {
        if words.is_empty() {
            tprintln!(
                term,
                "Please enter mnemonic (12 or 24 space separated words)"
            );
            ...
            tprintln!(
                term,
                "Please enter additional {} or {} words or <enter> to abort",
                remains_for_12,
                remains_for_24
            );
        } else {
            ...
            tprintln!(
                term,
                "Please enter additional {} words or <enter> to abort",
                remains_for_24
            );
        }
...
pub(crate) async fn import_with_mnemonic(
    ...
    match account_kind.as_ref() {
        LEGACY_ACCOUNT_KIND if length != 12 => {
            Err(Error::Custom(format!("wrong mnemonic length ({length})")))
        }
        BIP32_ACCOUNT_KIND if length != 24 => {
            Err(Error::Custom(format!("wrong mnemonic length ({length})")))
        }
    ...
    let payment_secret = if account_kind == LEGACY_ACCOUNT_KIND {
        ...
        let payment_secret = term
            .ask(true, "Enter payment password (optional): ")
            .await?;
    ...
    let account = if account_kind != MULTISIG_ACCOUNT_KIND {
        ...
        while matches!(
            term.ask(
                false,
                "Do you want to add more mnemonics (type 'y' to approve)?: "
            )
            .await?
            .trim(),
            "y" | "Y" | "YES" | "yes"
            ...
            let payment_secret = payment_secret
                .trim()
                .is_not_empty()
                .then(|| Secret::new(payment_secret.trim().as_bytes().to_vec()));
            let mnemonic = mnemonic.join(" ");
            let mnemonic = Mnemonic::new(mnemonic.trim(), Language::English)?;
        ...
        if additional_xpubs.is_empty() {
            loop {
                let xpub_key = term
                    .ask(false, "Enter extended public key: (empty to skip or stop)")
                    .await?;
