...
pub(crate) async fn create(
    ...
    let name = if let Some(name) = name {
        ...
        Some(
            term.ask(
                false,
                "Please enter account name (optional, press <enter> to skip): ",
            )
            .await?
            .trim()
            .to_string(),
    ...
    let wallet_secret = Secret::new(
        term.ask(true, "Enter wallet password: ")
            .await?
            .trim()
            .as_bytes()
            .to_vec(),
    ...
    let payment_secret = if prv_key_data_info.is_encrypted() {
        let payment_secret = Secret::new(
            term.ask(true, "Enter payment password: ")
                .await?
                .trim()
                .as_bytes()
                .to_vec(),
    ...
    let account = wallet
        .create_account_bip32(
            &wallet_secret,
            prv_key_data_info.id,
            payment_secret.as_ref(),
            account_create_args_bip32,
        )
        .await?;
...
async fn create_multisig(
    ...
    let minimum_signatures: u16 = term
        .ask(false, "Enter the minimum number of signatures required: ")
        .await?
        .parse()?;

    let prv_keys_len: usize = term
        .ask(false, "Enter the number of private keys to generate: ")
        .await?
        .parse()?;

    let mut prv_key_data_args = Vec::with_capacity(prv_keys_len);
    for _ in 0..prv_keys_len {
        let bip39_mnemonic = Secret::from(
            Mnemonic::random(mnemonic_phrase_word_count, Language::default())?.phrase(),
        );
        ...
        let prv_key_data_id = wallet
            .create_prv_key_data(&wallet_secret, prv_key_data_create_args)
            .await?;

        prv_key_data_args.push(PrvKeyDataArgs::new(prv_key_data_id, None));
    }

    let additional_xpub_keys_len: usize = term
        .ask(
            false,
            "Enter the number of additional extended public keys: ",
        )
        .await?
        .parse()?;
    ...
    for i in 1..=additional_xpub_keys_len {
        let xpub_key = term
            .ask(false, &format!("Enter extended public {i} key: "))
            .await?;
