...
pub fn register_link_matchers(cli: &Arc<KarlsenCli>) -> Result<()> {
    ...
    // addresses (open,copy) https://explorer.karlsen.org/addresses/
    let cli_ = cli.clone();
    cli.term().register_link_matcher(
        &js_sys::RegExp::new(r"(karlsen|karlsentest):\S+", "i"),
    ...
    // blocks (open,copy) https://explorer.karlsencoin.com/blocks/
    let cli_ = cli.clone();
    cli.term().register_link_matcher(
