...
use karlsen_consensus_notify::{
    notification as consensus_notification, notification::Notification as ConsensusNotification,
};
use karlsen_core::{debug, trace};
use karlsen_index_core::notification::{
    Notification, PruningPointUtxoSetOverrideNotification, UtxosChangedNotification,
};
use karlsen_notify::{
...
impl Processor {
    ...
    ) -> IndexResult<Notification> {
        match notification {
            ConsensusNotification::UtxosChanged(utxos_changed) => Ok(Notification::UtxosChanged(
                self.process_utxos_changed(utxos_changed).await?,
            )),
            ConsensusNotification::PruningPointUtxoSetOverride(_) => {
                Ok(Notification::PruningPointUtxoSetOverride(
    ...
    async fn process_utxos_changed(
        ...
        if let Some(utxoindex) = self.utxoindex.clone() {
            let converted_notification: UtxosChangedNotification = utxoindex
                .update(
                    notification.accumulated_utxo_diff.clone(),
                    notification.virtual_parents,
                )
                .await?
                .into();
            debug!(
...
mod tests {
    ...
    use karlsen_consensus::{
        config::Config, consensus::test_consensus::TestConsensus, params::DEVNET_PARAMS,
        test_helpers::*,
    };
    ...
    impl NotifyPipeline {
        fn new() -> Self {
            ...
            let consensus_manager =
                Arc::new(ConsensusManager::from_consensus(tc.consensus_clone()));
            let utxoindex = Some(UtxoIndexProxy::new(
                UtxoIndex::new(consensus_manager, utxoindex_db).unwrap(),
            ));
    ...
    async fn test_utxos_changed_notification() {
        ...
        let test_notification = consensus_notification::UtxosChangedNotification::new(
            Arc::new(UtxoDiff {
                add: to_add_collection,
                remove: to_remove_collection,
            }),
        ...
        pipeline
            .consensus_sender
            .send(ConsensusNotification::UtxosChanged(
                test_notification.clone(),
            ))
            .await
        ...
        match pipeline
            ...
            .recv()
            .await
            .expect("receives a notification")
        {
            Notification::UtxosChanged(utxo_changed_notification) => {
                let mut notification_utxo_added_count = 0;
                for (script_public_key, compact_utxo_collection) in
                    utxo_changed_notification.added.iter()
                {
                ...
                assert_eq!(
                    test_notification.accumulated_utxo_diff.add.len(),
                    notification_utxo_added_count
                );

                let mut notification_utxo_removed_count = 0;
                for (script_public_key, compact_utxo_collection) in
                    utxo_changed_notification.removed.iter()
                {
                    for (transaction_outpoint, compact_utxo) in compact_utxo_collection.iter() {
                        let test_utxo = test_notification
                            .accumulated_utxo_diff
                ...
                assert_eq!(
                    test_notification.accumulated_utxo_diff.remove.len(),
                    notification_utxo_removed_count
                );
            }
            unexpected_notification => {
                panic!("Unexpected notification: {unexpected_notification:?}")
            }
        }
        assert!(
            pipeline.processor_receiver.is_empty(),
            "the notification receiver should be empty"
        );
        ...
        pipeline
            ...
            .join()
            .await
            .expect("stopping the processor must succeed");
    }
    ...
    async fn test_pruning_point_utxo_set_override_notification() {
        ...
        pipeline
            .consensus_sender
            .send(ConsensusNotification::PruningPointUtxoSetOverride(
                test_notification.clone(),
            ))
            .await
            .expect("expected send");
