...
impl VirtualChangeEmulator {
    ...
    pub fn fill_utxo_collection(&mut self, amount: usize, script_public_key_pool_size: usize) {
        ...
        self.script_public_key_pool.extend(
            (0..script_public_key_pool_size).map(|_| generate_random_p2pk_script_public_key(rng)),
        );
        self.utxo_collection = generate_random_utxos_from_script_public_key_pool(
            rng,
            amount,
    ...
    pub fn change_virtual_state(
        ...
        self.accumulated_utxo_diff = Arc::new(UtxoDiff::new(
            UtxoCollection::from_iter(
                generate_random_utxos_from_script_public_key_pool(
                    rng,
                    add_amount,
                    &self.script_public_key_pool,
                )
            ...
            UtxoCollection::from_iter(self.utxo_collection.iter().take(remove_amount).map(
                |(k, v)| {
                    new_circulating_supply_diff -= v.amount as CirculatingSupplyDiff;
                    (*k, v.clone())
                },
        ...
        self.utxo_collection
            .retain(|k, _| !self.accumulated_utxo_diff.remove.contains_key(k));
        self.utxo_collection.extend(
            self.accumulated_utxo_diff
                .add
                .iter()
                .map(move |(k, v)| (*k, v.clone())),
        );
    ...
    pub fn clear_virtual_state(&mut self) {
        self.accumulated_utxo_diff =
            Arc::new(UtxoDiff::new(UtxoCollection::new(), UtxoCollection::new()));
