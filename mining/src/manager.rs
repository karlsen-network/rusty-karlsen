use crate::{
    ...
    mempool::{
        config::Config,
        model::tx::{
            MempoolTransaction, TransactionPostValidation, TransactionPreValidation,
            TxRemovalReason,
        },
...
impl MiningManager {
    ...
    pub fn get_block_template(
        ...
        loop {
            ...
            match block_template_builder
                .build_block_template(consensus, miner_data, selector, build_mode)
            {
                Ok(block_template) => {
                ...
                Err(BuilderError::ConsensusError(
                    ...
                    invalid_transactions.iter().for_each(|(x, err)| {
                        ...
                        if let Err(err) = removal_result {
                            ...
                            // https://github.com/karlsennet/karlsend/issues/1553
    ...
    pub(crate) fn get_realtime_feerate_estimations_verbose(
        &self,
        consensus: &dyn ConsensusApi,
        prefix: karlsen_addresses::Prefix,
        ...
        // calculate next_block_template_feerate_xxx
        {
            let script_public_key =
                karlsen_txscript::pay_to_address_script(&karlsen_addresses::Address::new(
                    prefix,
            ...
            let BlockTemplate {
                block: karlsen_consensus_core::block::MutableBlock { transactions, .. },
                calculated_fees,
                ..
            } = self.get_block_template(consensus, &miner_data)?;

            let Some(Stats { max, median, min }) = feerate_stats(transactions, calculated_fees)
            else {
                return Ok(resp);
            };
    ...
    pub(crate) fn validate_and_insert_mutable_transaction(
        ...
        let TransactionPreValidation {
            mut transaction,
            feerate_threshold,
        } = self.mempool.read().pre_validate_and_populate_transaction(
            consensus,
            transaction,
            rbf_policy,
        )?;
        ...
        match mempool.post_validate_and_insert_transaction(
            ...
            TransactionPostValidation {
                removed,
                accepted: Some(accepted_transaction),
            } => {
                ...
                let mut accepted_transactions =
                    Vec::with_capacity(unorphaned_transactions.len() + 1);
                // We include the original accepted transaction as well
                accepted_transactions.push(accepted_transaction);
                accepted_transactions.extend(self.validate_and_insert_unorphaned_transactions(
                    consensus,
...
impl MiningManagerProxy {
    ...
    pub async fn get_realtime_feerate_estimations(self) -> FeerateEstimations {
        spawn_blocking(move || self.inner.get_realtime_feerate_estimations())
            .await
            .unwrap()
    }
    ...
    pub async fn get_realtime_feerate_estimations_verbose(
        ...
        consensus
            ...
            .spawn_blocking(move |c| {
                self.inner
                    .get_realtime_feerate_estimations_verbose(c, prefix)
            })
            .await
    }
    ...
    pub async fn validate_and_insert_transaction(
        ...
        consensus
            .clone()
            .spawn_blocking(move |c| {
                self.inner.validate_and_insert_transaction(
                    c,
                    transaction,
                    priority,
                    orphan,
    ...
    pub async fn validate_and_insert_transaction_batch(
        ...
        consensus
            .clone()
            .spawn_blocking(move |c| {
                self.inner.validate_and_insert_transaction_batch(
                    c,
                    transactions,
                    priority,
                    orphan,
...
fn feerate_stats(transactions: Vec<Transaction>, calculated_fees: Vec<u64>) -> Option<Stats> {
    ...
    fees_and_masses.sort_unstable_by(|(lhs_fee, lhs_mass), (rhs_fee, rhs_mass)| {
        lhs_fee.mul(rhs_mass).cmp(&rhs_fee.mul(lhs_mass))
    });
...
mod tests {
    use super::*;
    use karlsen_consensus_core::subnets;
    use std::iter::repeat;
