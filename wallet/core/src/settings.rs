...
pub enum WalletSettings {
    #[describe("Network type (mainnet|testnet-1|testnet-11)")]
    Network,
    #[describe("Server address (default: 127.0.0.1)")]
    Server,
    #[describe("Wallet storage or file name (default 'karlsen')")]
    Wallet,
}
...
/// Returns the wallet data storage folder `~/.karlsen`.
pub fn application_folder() -> Result<PathBuf> {
    Ok(fs::resolve_path(storage::local::default_storage_folder())?)
}

/// If missing, creates the wallet data storage folder `~/.karlsen`.
