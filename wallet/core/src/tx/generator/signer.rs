    ...
    fn ingest(&self, addresses: &[Address]) -> Result<()> {
        ...
        if !addresses.is_empty() {
            let account = self
                ...
                .expect("expecting derivation capable account");
            let (receive, change) = account.derivation().addresses_indexes(&addresses)?;
            let private_keys = account.create_private_keys(
                &self.inner.keydata,
...
impl SignerT for Signer {
    fn try_sign(
        ...
        let mut keys_for_signing = addresses
            .iter()
            .map(|address| *keys.get(address).unwrap())
            .collect::<Vec<_>>();
...
impl KeydataSigner {
    pub fn new(keydata: Vec<(Address, secp256k1::SecretKey)>) -> Self {
        let keys = keydata
            .into_iter()
            .map(|(address, key)| (address, key.to_bytes()))
            .collect();
        Self {
            inner: Arc::new(KeydataSignerInner { keys }),
        }
    }
}
...
impl SignerT for KeydataSigner {
    fn try_sign(
        ...
        let mut keys_for_signing = addresses
            .iter()
            .map(|address| *self.inner.keys.get(address).unwrap())
            .collect::<Vec<_>>();
        // TODO - refactor for multisig
        let signable_tx = sign_with_multiple_v2(mutable_tx, &keys_for_signing).fully_signed()?;
        keys_for_signing.zeroize();
