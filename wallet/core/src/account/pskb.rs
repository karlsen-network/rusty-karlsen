...
use futures::stream;
use karlsen_bip32::{DerivationPath, KeyFingerprint, PrivateKey};
use karlsen_consensus_client::UtxoEntry as ClientUTXO;
...
use karlsen_consensus_core::tx::{TransactionInput, UtxoEntry};
use karlsen_txscript::extract_script_pub_key_address;
use karlsen_txscript::opcodes::codes::OpData65;
use karlsen_txscript::script_builder::ScriptBuilder;
use karlsen_wallet_core::tx::{Generator, GeneratorSettings, PaymentDestination, PendingTransaction};
pub use karlsen_wallet_pskt::bundle::Bundle;
use karlsen_wallet_pskt::bundle::{script_sig_to_address, unlock_utxo_outputs_as_batch_transaction_pskb};
use karlsen_wallet_pskt::prelude::lock_script_sig_templating_bytes;
use karlsen_wallet_pskt::prelude::KeySource;
use karlsen_wallet_pskt::prelude::{Finalizer, Inner, SignInputOk, Signature, Signer};
pub use karlsen_wallet_pskt::pskt::{Creator, PSKT};
...
pub fn pskt_to_pending_transaction(
    ...
    let output: &Vec<karlsen_consensus_core::tx::TransactionOutput> = &signed_tx.outputs;
    if output.is_empty() {
    ...
    let pending_tx = PendingTransaction::try_new(
        ...
        0,
        0,
        karlsen_wallet_core::tx::DataKind::Final,
    )?;
