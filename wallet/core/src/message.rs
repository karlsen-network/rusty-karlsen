...
use karlsen_hashes::{Hash, PersonalMessageSigningHash};
...
mod tests {
    ...
    fn sign_message_with_aux_rand(
        ...
        let sig: [u8; 64] = *curve
            .sign_schnorr_with_aux_rand(&msg, &schnorr_key, aux_rand)
            .as_ref();
    ...
    fn test_basic_sign_and_verify_sign() {
        let pm = PersonalMessage("Hello Karlsen!");
        let privkey: [u8; 32] = [
        ...
        verify_message(
            &pm,
            &sign_message(&pm, &privkey).expect("sign_message failed"),
            &pubkey,
        )
    ...
    fn test_kanji_sign_and_verify_sign() {
        ...
        let privkey: [u8; 32] = [
            0xB7, 0xE1, 0x51, 0x62, 0x8A, 0xED, 0x2A, 0x6A, 0xBF, 0x71, 0x58, 0x80, 0x9C, 0xF4,
            0xF3, 0xC7, 0x62, 0xE7, 0x16, 0x0F, 0x38, 0xB4, 0xDA, 0x56, 0xA7, 0x84, 0xD9, 0x04,
            0x51, 0x90, 0xCF, 0xEF,
        ];
        let pubkey = XOnlyPublicKey::from_slice(&[
...
Ut omnis magnam et accusamus earum rem impedit provident eum commodi repellat qui dolores quis et voluptate labore et adipisci deleniti. Est nostrum explicabo aut quibusdam labore et molestiae voluptate. Qui omnis nostrum At libero deleniti et quod quia.");
    ...
    #[test]
    fn test_fail_verify() {
        let pm = PersonalMessage("Not Hello Karlsen!");
        let pubkey = XOnlyPublicKey::from_slice(&[
            0xF9, 0x30, 0x8A, 0x01, 0x92, 0x58, 0xC3, 0x10, 0x49, 0x34, 0x4F, 0x85, 0xF8, 0x9D,
            0x52, 0x29, 0xB5, 0x31, 0xC8, 0x45, 0x83, 0x6F, 0x99, 0xB0, 0x86, 0x01, 0xF1, 0x13,
            0xBC, 0xE0, 0x36, 0xF9,
    ...
    fn test_sign_and_verify_test_case_0() {
        ...
        let expected_sig: Vec<u8> = [
            0x0F, 0xCF, 0x43, 0xD8, 0x38, 0x87, 0xE7, 0x60, 0xE6, 0x6E, 0xCF, 0xE9, 0x8B, 0x93,
            0x9F, 0xEE, 0xD4, 0x08, 0x59, 0xED, 0x6D, 0x4F, 0x3F, 0xF9, 0x83, 0x68, 0x1E, 0xE1,
            0xAC, 0x5F, 0x68, 0xB7, 0x70, 0x8A, 0x49, 0xDB, 0xB0, 0xE4, 0x13, 0x11, 0x86, 0x1E,
            0x50, 0x40, 0x6B, 0xC4, 0x68, 0xA8, 0xFC, 0x07, 0x35, 0xC3, 0x7F, 0x93, 0x7B, 0x92,
            0x27, 0xF4, 0xA6, 0x35, 0x75, 0x78, 0x07, 0xEB,
        ]
        .to_vec();

        let sig_result =
            sign_message_with_aux_rand(&pm, &privkey, &aux_rand).expect("sign_message failed");
    ...
    fn test_sign_and_verify_test_case_1() {
        ...
        .unwrap();
        let expected_sig: Vec<u8> = [
            0xA0, 0x80, 0x8C, 0xDB, 0x22, 0xFE, 0x9B, 0x3D, 0x0C, 0x2B, 0xB6, 0x35, 0x6F, 0xE5,
            0x0C, 0xFA, 0x74, 0x19, 0xC0, 0x20, 0xFB, 0xAB, 0x25, 0x0C, 0x94, 0x42, 0xD1, 0x09,
            0xC5, 0xD8, 0x67, 0xF9, 0x28, 0xF1, 0x36, 0x87, 0xB2, 0x3A, 0xF9, 0x5F, 0x2D, 0x46,
            0x88, 0x8D, 0x26, 0x7B, 0x93, 0x5C, 0x6C, 0x88, 0xDF, 0x0E, 0x77, 0x66, 0x15, 0x8F,
            0xCE, 0x78, 0x2B, 0xAC, 0xBF, 0xB3, 0x3F, 0xC2,
        ]
        .to_vec();
    ...
    fn test_sign_and_verify_test_case_2() {
        ...
        .unwrap();
        let expected_sig: Vec<u8> = [
            0x81, 0x06, 0x53, 0xD5, 0xF8, 0x02, 0x06, 0xDB, 0x51, 0x96, 0x72, 0x36, 0x2A, 0xDD,
            0x6C, 0x98, 0xDA, 0xD3, 0x78, 0x84, 0x4E, 0x5B, 0xA4, 0xD8, 0x9A, 0x22, 0xC9, 0xF0,
            0xC7, 0x09, 0x2E, 0x8C, 0xEC, 0xBA, 0x73, 0x4F, 0xFF, 0x79, 0x22, 0xB6, 0x56, 0xB4,
            0xBE, 0x3F, 0x4B, 0x1F, 0x09, 0x88, 0x99, 0xC9, 0x5C, 0xB5, 0xC1, 0x02, 0x3D, 0xCE,
            0x35, 0x19, 0x20, 0x8A, 0xFA, 0xFB, 0x59, 0xBC,
        ]
        .to_vec();
...
Ut omnis magnam et accusamus earum rem impedit provident eum commodi repellat qui dolores quis et voluptate labore et adipisci deleniti. Est nostrum explicabo aut quibusdam labore et molestiae voluptate. Qui omnis nostrum At libero deleniti et quod quia.");
        ...
        .unwrap();
        let expected_sig: Vec<u8> = [
            0x40, 0xCB, 0xBD, 0x39, 0x38, 0x86, 0x7B, 0x10, 0x07, 0x6B, 0xB1, 0x48, 0x35, 0x55,
            0x7C, 0x06, 0x2F, 0x5B, 0xF6, 0xA4, 0x68, 0x29, 0x95, 0xFC, 0x8B, 0x0A, 0x1C, 0xD2,
            0xED, 0x98, 0x6E, 0xED, 0xAA, 0xA0, 0x0C, 0xFE, 0x04, 0xF6, 0xC9, 0xE5, 0xA9, 0x54,
            0x6B, 0x86, 0x07, 0x32, 0xE5, 0xB9, 0x03, 0xCC, 0x82, 0x78, 0x02, 0x28, 0x64, 0x7D,
            0x53, 0x75, 0xBE, 0xC3, 0xD2, 0xA4, 0x98, 0x3A,
        ]
        .to_vec();
