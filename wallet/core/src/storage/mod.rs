...
pub use id::IdT;
pub use interface::{
    AccountStore, Interface, PrvKeyDataStore, StorageDescriptor, TransactionRecordStore,
    WalletDescriptor, WalletExportOptions,
};
pub use keydata::{
    AssocPrvKeyDataIds, PrvKeyData, PrvKeyDataId, PrvKeyDataInfo, PrvKeyDataMap, PrvKeyDataPayload,
};
...
mod tests {
    ...
    async fn test_storage_wallet_store_load() -> Result<()> {
        ...
        let prv_key_data1 = PrvKeyData::try_new_from_mnemonic(
            mnemonic1.clone(),
            Some(&payment_secret),
            EncryptionKind::XChaCha20Poly1305,
        )?;
        let pub_key_data1 = prv_key_data1
            .create_xpub(Some(&payment_secret), BIP32_ACCOUNT_KIND.into(), 0)
            .await?;

        let mnemonic2 = Mnemonic::new(mnemonic2s.clone(), Language::English)?;
        let prv_key_data2 = PrvKeyData::try_new_from_mnemonic(
            mnemonic2.clone(),
            Some(&payment_secret),
            EncryptionKind::XChaCha20Poly1305,
        )?;
        let pub_key_data2 = prv_key_data2
            .create_xpub(Some(&payment_secret), BIP32_ACCOUNT_KIND.into(), 0)
            .await?;
        ...
        let account1 = AccountStorage::try_new(
        ...
        let account2 = AccountStorage::try_new(
            BIP32_ACCOUNT_KIND.into(),
            &id,
            &storage_key,
        ...
        let first_mnemonic = &w2keydata1_payload
            .as_ref()
            .as_mnemonic()?
            .unwrap()
            .phrase_string();
        ...
        let second_mnemonic = &w2keydata2_payload
            .as_ref()
            .as_mnemonic()?
            .unwrap()
            .phrase_string();
