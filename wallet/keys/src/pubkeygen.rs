...
use crate::imports::*;
use karlsen_addresses::AddressArrayT;
...
impl PublicKeyGenerator {
    ...
    pub fn receive_pubkeys(&self, mut start: u32, mut end: u32) -> Result<PublicKeyArrayT> {
        ...
        Ok(Array::from_iter(
            pubkeys
                .into_iter()
                .map(|pk| JsValue::from(PublicKey::from(pk))),
        )
        .unchecked_into())
    }
    ...
    pub fn receive_pubkeys_as_strings(&self, mut start: u32, mut end: u32) -> Result<StringArray> {
        ...
        Ok(Array::from_iter(
            pubkeys
                .into_iter()
                .map(|pk| JsValue::from(PublicKey::from(pk).to_string())),
        )
        .unchecked_into())
    }
    ...
    #[allow(non_snake_case)]
    pub fn receive_addresses_as_strings(
        &self,
        networkType: &NetworkTypeT,
        mut start: u32,
        mut end: u32,
        ...
        let addresses = pubkeys
            ...
            .collect::<Result<Vec<Address>>>()?;
        Ok(
            Array::from_iter(addresses.into_iter().map(String::from).map(JsValue::from))
                .unchecked_into(),
        )
    }
    ...
    #[allow(non_snake_case)]
    pub fn receive_address_as_string(
        &self,
        networkType: &NetworkTypeT,
        index: u32,
    ) -> Result<String> {
