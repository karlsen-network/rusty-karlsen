...
pub trait RpcApi: Sync + Send + AnySync {
    ...
    async fn get_sync_status(&self) -> RpcResult<bool> {
        Ok(self
            .get_sync_status_call(GetSyncStatusRequest {})
            .await?
            .is_synced)
    }
    ...
    async fn submit_block(
        ...
        self.submit_block_call(SubmitBlockRequest::new(block, allow_non_daa_blocks))
            .await
    }
    ...
    async fn get_block_template(
        ...
        self.get_block_template_call(GetBlockTemplateRequest::new(pay_address, extra_data))
            .await
    }
    ...
    async fn get_peer_addresses(&self) -> RpcResult<GetPeerAddressesResponse> {
        self.get_peer_addresses_call(GetPeerAddressesRequest {})
            .await
    }
    ...
    async fn get_mempool_entry(
        ...
        Ok(self
            .get_mempool_entry_call(GetMempoolEntryRequest::new(
                transaction_id,
                include_orphan_pool,
    ...
    async fn get_mempool_entries(
        ...
        Ok(self
            .get_mempool_entries_call(GetMempoolEntriesRequest::new(
                include_orphan_pool,
                filter_transaction_pool,
            ))
            .await?
    ...
    async fn get_connected_peer_info(&self) -> RpcResult<GetConnectedPeerInfoResponse> {
        self.get_connected_peer_info_call(GetConnectedPeerInfoRequest {})
            .await
    }
    ...
    async fn add_peer(
        ...
        self.add_peer_call(AddPeerRequest::new(peer_address, is_permanent))
            .await?;
        Ok(())
    }
    ...
    async fn submit_transaction(
        ...
        allow_orphan: bool,
    ) -> RpcResult<RpcTransactionId> {
        Ok(self
            .submit_transaction_call(SubmitTransactionRequest {
                transaction,
                allow_orphan,
            })
            .await?
    ...
    async fn submit_transaction_replacement(
        ...
        self.submit_transaction_replacement_call(SubmitTransactionReplacementRequest {
            transaction,
        })
        .await
    }
    ...
    async fn get_block(&self, hash: RpcHash, include_transactions: bool) -> RpcResult<RpcBlock> {
        Ok(self
            .get_block_call(GetBlockRequest::new(hash, include_transactions))
            .await?
            .block)
    }
    ...
    async fn get_subnetwork(
        ...
        self.get_subnetwork_call(GetSubnetworkRequest::new(subnetwork_id))
            .await
    }
    ...
    async fn get_virtual_chain_from_block(
        ...
        self.get_virtual_chain_from_block_call(GetVirtualChainFromBlockRequest::new(
            start_hash,
    ...
    async fn get_blocks(
        ...
        self.get_blocks_call(GetBlocksRequest::new(
            low_hash,
            include_blocks,
    ...
    async fn get_block_dag_info(&self) -> RpcResult<GetBlockDagInfoResponse> {
        self.get_block_dag_info_call(GetBlockDagInfoRequest {})
            .await
    }
    ...
    async fn resolve_finality_conflict(&self, finality_block_hash: RpcHash) -> RpcResult<()> {
        self.resolve_finality_conflict_call(ResolveFinalityConflictRequest::new(
            finality_block_hash,
        ))
        .await?;
        Ok(())
    ...
    async fn get_headers(
        ...
        limit: u64,
        is_ascending: bool,
    ) -> RpcResult<Vec<RpcHeader>> {
        Ok(self
            .get_headers_call(GetHeadersRequest::new(start_hash, limit, is_ascending))
            .await?
            .headers)
    }
    ...
    async fn get_balance_by_address(&self, address: RpcAddress) -> RpcResult<u64> {
        Ok(self
            .get_balance_by_address_call(GetBalanceByAddressRequest::new(address))
            .await?
            .balance)
    }
    ...
    ) -> RpcResult<Vec<RpcBalancesByAddressesEntry>> {
        Ok(self
            .get_balances_by_addresses_call(GetBalancesByAddressesRequest::new(addresses))
            .await?
            .entries)
    }
    ...
    async fn get_utxos_by_addresses(
        ...
        Ok(self
            .get_utxos_by_addresses_call(GetUtxosByAddressesRequest::new(addresses))
            .await?
            .entries)
    }
    ...
    async fn estimate_network_hashes_per_second(
        ...
        Ok(self
            .estimate_network_hashes_per_second_call(EstimateNetworkHashesPerSecondRequest::new(
                window_size,
                start_hash,
            ))
            .await?
    ...
    async fn get_fee_estimate(&self) -> RpcResult<RpcFeeEstimate> {
        Ok(self
            .get_fee_estimate_call(GetFeeEstimateRequest {})
            .await?
            .estimate)
    }
    ...
    async fn get_fee_estimate_experimental(
        &self,
        verbose: bool,
    ) -> RpcResult<GetFeeEstimateExperimentalResponse> {
        self.get_fee_estimate_experimental_call(GetFeeEstimateExperimentalRequest { verbose })
            .await
    }
