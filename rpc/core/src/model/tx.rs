...
use karlsen_utils::{hex::ToHex, serde_bytes_fixed_ref};
...
impl From<karlsen_consensus_client::TransactionOutpoint> for RpcTransactionOutpoint {
    fn from(outpoint: karlsen_consensus_client::TransactionOutpoint) -> Self {
        TransactionOutpoint::from(outpoint).into()
    }
}

impl From<RpcTransactionOutpoint> for karlsen_consensus_client::TransactionOutpoint {
    fn from(outpoint: RpcTransactionOutpoint) -> Self {
        TransactionOutpoint::from(outpoint).into()
    }
}
