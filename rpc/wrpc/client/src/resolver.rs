//!
//! Module implementing [`Resolver`] client for obtaining public Karlsen wRPC endpoints.
//!
...
impl Resolver {
    ...
    fn make_url(&self, url: &str, encoding: Encoding, network_id: NetworkId) -> String {
        ...
        format!("{url}/v{CURRENT_VERSION}/karlsen/{network_id}/{tls}/wrpc/{encoding}")
    }
    ...
    /// Obtain a Karlsen p2p [`NodeDescriptor`] from the resolver based on the supplied [`Encoding`] and [`NetworkId`].
    ...
    /// Returns a Karlsen wRPC URL from the resolver based on the supplied [`Encoding`] and [`NetworkId`].
