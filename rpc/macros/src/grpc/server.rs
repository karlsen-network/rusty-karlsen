...
struct RpcTable {
    ...
    karlsend_request_type: Expr,
    karlsend_response_type: Expr,
    payload_ops: Expr,
    handlers: ExprArray,
}
    ...
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        if parsed.len() != 7 {
            return Err(Error::new_spanned(parsed,
                "usage: build_grpc_server_interface!(server_context, ServerContextType, ConnectionType, KarlsendRequestType, KarlsendResponseType, KarlsendPayloadOps, [GetInfo, ..])".to_string()));
        }
        ...
        let karlsend_request_type = iter.next().unwrap().clone();
        let karlsend_response_type = iter.next().unwrap().clone();
        ...
        Ok(RpcTable {
            ...
            server_ctx_type,
            connection_ctx_type,
            karlsend_request_type,
            karlsend_response_type,
            payload_ops,
            handlers,
        })
    }
...
impl ToTokens for RpcTable {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let karlsend_request_type = &self.karlsend_request_type;
        let karlsend_response_type = &self.karlsend_response_type;
        ...
        for handler in self.handlers.elems.iter() {
            ...
            match is_subscription {
                false => {
                    targets.push(quote! {
                        #payload_ops::#handler => {
                            let method: Method<#server_ctx_type, #connection_ctx_type, #karlsend_request_type, #karlsend_response_type> =
                            Method::new(|server_ctx: #server_ctx_type, _: #connection_ctx_type, request: #karlsend_request_type| {
                                Box::pin(async move {
                ...
                true => {
                    targets.push(quote! {
                        #payload_ops::#handler => {
                            let method: Method<#server_ctx_type, #connection_ctx_type, #karlsend_request_type, #karlsend_response_type> =
                            Method::new(|server_ctx: #server_ctx_type, connection: #connection_ctx_type, request: #karlsend_request_type| {
                                Box::pin(async move {
                                    let mut response: #karlsend_response_type = match request.payload {
                                        Some(Payload::#request_type(ref request)) => {
                                            match karlsen_rpc_core::#fallback_request_type::try_from(request) {
                                                Ok(request) => {
