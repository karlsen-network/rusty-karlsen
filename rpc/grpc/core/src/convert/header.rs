...
from!(item: &karlsen_rpc_core::RpcRawHeader, protowire::RpcBlockHeader, {
    Self {
        version: item.version.into(),
        parents: item.parents_by_level.iter().map(protowire::RpcBlockLevelParents::from).collect(),
        hash_merkle_root: item.hash_merkle_root.to_string(),
...
try_from!(item: &protowire::RpcBlockHeader, karlsen_rpc_core::RpcRawHeader, {
    Self {
        ...
        nonce: item.nonce,
        daa_score: item.daa_score,
        blue_work: karlsen_rpc_core::RpcBlueWorkType::from_rpc_hex(&item.blue_work)?,
        blue_score: item.blue_score,
...
mod tests {
    ...
    use karlsen_consensus_core::{block::Block, header::Header};
    use karlsen_rpc_core::{RpcBlock, RpcHash, RpcHeader};
