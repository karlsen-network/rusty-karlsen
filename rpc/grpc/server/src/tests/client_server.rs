...
use karlsen_notify::scope::{NewBlockTemplateScope, Scope};
use karlsen_rpc_core::api::rpc::RpcApi;
use karlsen_utils::networking::{ContextualNetAddress, NetAddress};
use std::sync::Arc;

#[tokio::test]
async fn test_client_server_sanity_check() {
    karlsen_core::log::try_init_logger(
        "info, karlsen_grpc_core=trace, karlsen_grpc_server=trace, karlsen_grpc_client=trace",
    );
    ...
    assert!(
        ...
        "server should have no client when just started"
    );
    ...
    assert!(
        client.handle_message_id() && client.handle_stop_notify(),
        ...
        async fn execute(&self) {
            ...
            for (i, closing) in self.ends.iter().enumerate() {
                match *closing {
                    ClosingEnd::Client => {
                        assert!(
                            clients[i].disconnect().await.is_ok(),
                            "client {} failed to disconnect",
                            i
                        );
                        clients_left -= 1;
            ...
            if self.terminate_clients {
                ...
                assert!(
                    !server.has_connections(),
                    "server should have no more clients"
                );
            }
...
async fn test_client_server_notifications() {
    ...
    assert!(client1
        .start_notify(0, Scope::NewBlockTemplate(NewBlockTemplateScope::default()))
        .await
        .is_ok());
    assert!(client2
        .start_notify(0, Scope::NewBlockTemplate(NewBlockTemplateScope::default()))
    ...
    assert!(
        client1.disconnect().await.is_ok(),
        "client failed to disconnect"
    );
    drop(client1);
