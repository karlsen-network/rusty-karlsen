...
use crate::pb::{karlsend_message::Payload, ReadyMessage, VerackMessage, VersionMessage};
...
impl<'a> KarlsendHandshake<'a> {
    /// Builds the handshake object and subscribes to handshake messages
    pub fn new(router: &'a Router) -> Self {
        Self {
            router,
            version_receiver: router.subscribe(vec![KarlsendMessagePayloadType::Version]),
            verack_receiver: router.subscribe(vec![KarlsendMessagePayloadType::Verack]),
            ready_receiver: router.subscribe(vec![KarlsendMessagePayloadType::Ready]),
        }
    }
