...
use karlsen_consensus_core::errors::{consensus::ConsensusError, sync::SyncManagerError};
use karlsen_p2p_lib::{
...
impl RequestIbdChainBlockLocatorFlow {
    ...
    async fn start_impl(&mut self) -> Result<(), ProtocolError> {
        loop {
            let (msg, request_id) = dequeue_with_request_id!(
                self.incoming_route,
                Payload::RequestIbdChainBlockLocator
            )?;
            let (low, high) = msg.try_into()?;

            let locator = match (self.ctx.consensus().session().await)
                .async_create_virtual_selected_chain_block_locator(low, high)
                .await
            {
                Ok(locator) => Ok(locator),
                Err(e) => {
                    let orig = e.clone();
                    if let ConsensusError::SyncManagerError(
                        SyncManagerError::BlockNotInSelectedParentChain(_),
                    ) = e
                    {
            ...
            self.router
                .enqueue(make_response!(
                    ...
                    IbdChainBlockLocatorMessage {
                        block_locator_hashes: locator.into_iter().map(|hash| hash.into()).collect()
                    },
                    request_id
                ))
                .await?;
