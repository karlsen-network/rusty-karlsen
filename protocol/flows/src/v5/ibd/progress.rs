...
impl ProgressReporter {
    ...
    pub fn report(
        ...
        let now = Instant::now();
        if now - self.last_log_time < REPORT_TIME_GRANULARITY
        ...
        let relative_daa_score = if current_daa_score > self.low_daa_score {
            current_daa_score - self.low_daa_score
        } else {
            0
        };
        let percent = ((relative_daa_score as f64
            / (self.high_daa_score - self.low_daa_score) as f64)
            * 100.0) as i32;
        if percent > self.last_reported_percent {
            let date = match Local.timestamp_opt(
                current_timestamp as i64 / 1000,
                1000 * (current_timestamp as u32 % 1000),
            ) {
            ...
            info!(
                ...
                self.processed, self.object_name, percent, date
            );
    ...
    pub fn report_completion(mut self, processed_delta: usize) {
        ...
        info!(
            "IBD: Processed {} {} (100%)",
            self.processed, self.object_name
        );
    }
}
