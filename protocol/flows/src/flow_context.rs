...
impl FlowContext {
    ...
    pub async fn submit_rpc_block(&self, consensus: &ConsensusProxy, block: Block) -> Result<(), ProtocolError> {
    	...
    	info!("BlueWork[{}] BlueScore[{}] DAAScore[{}] Bits[{}] Version[{}]", block.header.blue_work, block.header.blue_score, block.header.daa_score, block.header.bits, block.header.version);
