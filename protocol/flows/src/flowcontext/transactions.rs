...
impl TransactionsSpread {
    ...
    pub fn should_run_mempool_scanning_task(&mut self) -> bool {
        let now = Instant::now();
        if self.scanning_task_running
            || now < self.last_scanning_time + Duration::from_secs(SCANNING_TASK_INTERVAL)
        {
            return false;
        }
        let delta = now
            .checked_duration_since(self.last_scanning_time)
        ...
        self.last_scanning_time += Duration::from_secs(
            ((delta.as_secs() + SCANNING_TASK_INTERVAL - 1) / SCANNING_TASK_INTERVAL)
                * SCANNING_TASK_INTERVAL,
        );
    ...
    pub fn mempool_scanning_is_done(&mut self) {
        assert!(
            self.scanning_task_running,
            "no stop without a matching start"
        );
    ...
    pub async fn broadcast_transactions<I: IntoIterator<Item = TransactionId>>(
        ...
        if now < self.last_broadcast_time + BROADCAST_INTERVAL
            && self.transaction_ids.len() < MAX_INV_PER_TX_INV_MSG
        {
            return;
        }
        ...
        while !self.transaction_ids.is_empty() {
            let ids = self
                .transaction_ids
                .dequeue_chunk(MAX_INV_PER_TX_INV_MSG)
                .map(|x| x.into())
                .collect_vec();
            debug!(
                "Transaction propagation: broadcasting {} transactions",
                ids.len()
            );
