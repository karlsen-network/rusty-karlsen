...
use karlsen_hashes::{Hash, Hasher};
...
pub fn hash(tx: &Transaction, include_mass_field: bool) -> Hash {
    let mut hasher = karlsen_hashes::TransactionHash::new();
...
pub(crate) fn id(tx: &Transaction) -> TransactionId {
    ...
    let mut hasher = karlsen_hashes::TransactionID::new();
...
fn write_input<T: Hasher>(
    ...
    {
        hasher
            .write_var_bytes(input.signature_script.as_slice())
            .update([input.sig_op_count]);
    } else {
...
mod tests {
    ...
    fn test_transaction_hashing() {
        ...
        let outputs = vec![TransactionOutput::new(
            1564,
            ScriptPublicKey::new(7, scriptvec![1, 2, 3, 4, 5]),
        )];
        ...
        let inputs = vec![TransactionInput::new(
            TransactionOutpoint::new(
                Hash::from_str("59b3d6dc6cdc660c389c3fdb5704c48c598d279cdf1bab54182db586a4c95dd5")
                    .unwrap(),
                2,
            ),
            vec![1, 2],
