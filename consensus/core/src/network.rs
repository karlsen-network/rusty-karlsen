...
impl FromStr for NetworkType {
    ...
    fn from_str(network_type: &str) -> Result<Self, Self::Err> {
        match network_type.to_lowercase().as_str() {
            ...
            _ => Err(NetworkTypeError::InvalidNetworkType(
...
extern "C" {
    #[wasm_bindgen(
        js_name = "Network",
        typescript_type = "NetworkType | NetworkId | string"
    )]
    #[derive(Debug)]
...
pub enum NetworkIdError {
    #[error("Invalid network name prefix: {0}. The expected prefix is 'karlsen'.")]
...
impl NetworkId {
    pub const fn new(network_type: NetworkType) -> Self {
        if !matches!(
            network_type,
            NetworkType::Mainnet | NetworkType::Devnet | NetworkType::Simnet
        ) {
    ...
    pub fn try_new(network_type: NetworkType) -> Result<Self, NetworkIdError> {
        if !matches!(
            ...
            return Err(NetworkIdError::NetworkSuffixRequired(
                network_type.to_string(),
            ));
        }
    ...
    pub fn default_p2p_port(&self) -> u16 {
        ...
        match self.network_type {
            NetworkType::Mainnet => 42111,
            NetworkType::Testnet => match self.suffix {
                Some(1) => 42211,
                Some(11) => 42711,
                None | Some(_) => 42211,
            },
            NetworkType::Simnet => 42511,
            NetworkType::Devnet => 42611,
        }
    }
    ...
    /// Returns a textual description of the network prefixed with `karlsen-`
    pub fn to_prefixed(&self) -> String {
        format!("karlsen-{}", self)
    }

    pub fn from_prefixed(prefixed: &str) -> Result<Self, NetworkIdError> {
        if let Some(stripped) = prefixed.strip_prefix("karlsen-") {
...
impl FromStr for NetworkId {
    ...
    fn from_str(network_name: &str) -> Result<Self, Self::Err> {
        ...
        let suffix = parts
            .next()
            .map(|x| u32::from_str(x).map_err(|_| NetworkIdError::InvalidSuffix(x.to_string())))
            .transpose()?;
        ...
        {
            return Err(NetworkIdError::MissingNetworkSuffix(
                network_name.to_string(),
            ));
        }
        match parts.next() {
            Some(extra_token) => Err(NetworkIdError::UnexpectedExtraToken(
                extra_token.to_string(),
            )),
            None => Ok(Self {
                network_type,
                suffix,
    ...
    fn try_cast_from(value: impl AsRef<JsValue>) -> Result<Cast<Self>, Self::Error> {
        Self::resolve(&value, || {
            if let Some(network_id) = value.as_ref().as_string() {
                ...
                Err(NetworkIdError::InvalidNetworkId(format!(
                    "{:?}",
                    value.as_ref()
                )))
            }
    ...
    fn test_network_id_parse() {
        ...
        let tests = vec![
            ...
            Test {
                name: "Invalid suffix",
                expr: "testnet-x",
                expected: Err(NetworkIdError::InvalidSuffix("x".to_string())),
            },
            Test {
