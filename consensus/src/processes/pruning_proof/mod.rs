...
impl<T: RelationsStoreReader, U: ReachabilityService> RelationsStoreReader for RelationsStoreInFutureOfRoot<T, U> {
    fn get_parents(&self, hash: Hash) -> Result<BlockHashes, karlsen_database::prelude::StoreError> {
    ...
    fn get_children(&self, hash: Hash) -> StoreResult<karlsen_database::prelude::ReadLock<BlockHashSet>> {
    ...
    fn counts(&self) -> Result<(usize, usize), karlsen_database::prelude::StoreError> {
        unimplemented!()
    }
}
...
impl PruningProofManager {
    ...
    pub(crate) fn build_pruning_point_proof(&self, pp: Hash) -> PruningPointProof {
        ...
        let (_db_lifetime, temp_db) = karlsen_database::create_temp_db!(ConnBuilder::default().with_files_limit(10));
        ...
        for anticone_block in anticone.iter().copied() {
            ...
            for hash in ghostdag_chain {
                if let Entry::Vacant(e) = ghostdag_blocks.entry(hash) {
                    ...
                    // We fill `ghostdag_blocks` only for karlsend-go legacy reasons, but the real set we
                    // send is `daa_window_blocks` which represents the full trusted sub-DAG in the antifuture
                    // of the pruning point which karlsend-rust nodes expect to get when synced with headers proof
                    if let Entry::Vacant(e) = daa_window_blocks.entry(hash) {
                        e.insert(TrustedHeader {
                            header: self.headers_store.get_header(hash).unwrap(),
