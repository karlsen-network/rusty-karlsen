...
pub fn generate_random_utxos_from_script_public_key_pool(
    ...
    while i < amount {
        collection.insert(
            ...
            generate_random_utxo_from_script_public_key_pool(rng, script_public_key_pool),
        );
        i += 1;
    }
    collection
...
pub fn generate_random_utxo_from_script_public_key_pool(
    ...
    UtxoEntry::new(
        ...
        script_public_key_pool
            .choose(rng)
            .expect("expected_script_public key")
            .clone(),
        rng.gen(),
        rng.gen_bool(0.5),
    )
...
pub fn generate_random_transactions(
    ...
    Vec::from_iter(
        (0..amount).map(move |_| generate_random_transaction(rng, input_amount, output_amount)),
    )
}
...
pub fn generate_random_transaction_inputs(
    rng: &mut SmallRng,
    amount: usize,
...
///Note: generate_random_transactions is filled with random data, it does not represent consensus-valid  transaction output!
pub fn generate_random_transaction_outputs(
    rng: &mut SmallRng,
    amount: usize,
