...
impl HeaderProcessor {
    ...
    pub fn check_median_timestamp(
        ...
        let (past_median_time, window) = self
            .window_manager
            .calc_past_median_time(ctx.ghostdag_data())?;
    ...
    pub fn check_merge_size_limit(
        ...
        if mergeset_size > self.mergeset_size_limit {
            return Err(RuleError::MergeSetTooBig(
                mergeset_size,
    ...
    fn check_blue_score(
        ...
        if gd_blue_score != header.blue_score {
            return Err(RuleError::UnexpectedHeaderBlueScore(
                gd_blue_score,
                header.blue_score,
            ));
        }
    ...
    fn check_blue_work(
        ...
        if gd_blue_work != header.blue_work {
            return Err(RuleError::UnexpectedHeaderBlueWork(
                gd_blue_work,
                header.blue_work,
            ));
        }
    ...
    pub fn check_indirect_parents(
        ...
        let expected_block_parents = self
            .parents_manager
            .calc_block_parents(ctx.pruning_point(), header.direct_parents());
        if header.parents_by_level.len() != expected_block_parents.len()
            || !expected_block_parents.iter().enumerate().all(
                |(block_level, expected_level_parents)| {
                    ...
                    header_level_parents
                        .iter()
                        .all(|header_parent| expected_set.contains(header_parent))
                },
            )
        {
    ...
    pub fn check_pruning_point(
        ...
        let expected = self
            ...
            .expected_header_pruning_point(ctx.ghostdag_data().to_compact(), ctx.pruning_info);
        if expected != header.pruning_point {
            return Err(RuleError::WrongHeaderPruningPoint(
                expected,
    ...
    pub fn check_bounded_merge_depth(
        ...
        let merge_depth_root = self
            .depth_manager
            .calc_merge_depth_root(ghostdag_data, ctx.pruning_point());
        let finality_point = self
            .depth_manager
            .calc_finality_point(ghostdag_data, ctx.pruning_point());
