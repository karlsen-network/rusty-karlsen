...
pub const UTXO_KEY_SIZE: usize = karlsen_hashes::HASH_SIZE + size_of::<TransactionIndexType>();
...
impl TryFrom<&[u8]> for UtxoKey {
    ...
    fn try_from(slice: &[u8]) -> Result<Self, Self::Error> {
        ...
        if slice.len() < karlsen_hashes::HASH_SIZE + 1 {
...
impl From<UtxoKey> for TransactionOutpoint {
    fn from(k: UtxoKey) -> Self {
        ...
        let index = TransactionIndexType::from_le_bytes(
            <[u8; size_of::<TransactionIndexType>()]>::try_from(&k.0[karlsen_hashes::HASH_SIZE..]).expect("expecting index size"),
        );
