...
use crate::{
    ...
    pipeline::{
        ...
        deps_manager::{
            BlockProcessingMessage, BlockResultSender, BlockTask, VirtualStateProcessingMessage,
        },
...
use crossbeam_channel::{
    bounded as bounded_crossbeam, unbounded as unbounded_crossbeam, Receiver as CrossbeamReceiver,
...
impl Consensus {
    pub fn new(
        ...
        let (sender, receiver): (
            CrossbeamSender<BlockProcessingMessage>,
            CrossbeamReceiver<BlockProcessingMessage>,
        ) = unbounded_crossbeam();
    ...
    pub fn run_processors(&self) -> Vec<JoinHandle<()>> {
        ...
        vec![
            thread::Builder::new()
                .name("header-processor".to_string())
                .spawn(move || header_processor.worker())
                .unwrap(),
            thread::Builder::new()
                .name("body-processor".to_string())
                .spawn(move || body_processor.worker())
                .unwrap(),
            thread::Builder::new()
                .name("virtual-processor".to_string())
                .spawn(move || virtual_processor.worker())
                .unwrap(),
            thread::Builder::new()
                .name("pruning-processor".to_string())
                .spawn(move || pruning_processor.worker())
                .unwrap(),
        ]
    }
    ...
    fn validate_and_insert_block_impl(
        ...
        self.block_sender
            .send(BlockProcessingMessage::Process(task, btx, vtx))
            .unwrap();
        self.counters
            .blocks_submitted
    ...
    pub fn signal_exit(&self) {
        ...
        self.block_sender
            .send(BlockProcessingMessage::Exit)
            .unwrap();
    }
    ...
    fn estimate_network_hashes_per_second_impl(
        ...
        let window = match self
            .services
            .window_manager
            .block_window(ghostdag_data, WindowType::VaryingWindow(window_size))
        {
            Ok(w) => w,
            Err(RuleError::InsufficientDaaWindowSize(s)) => {
                return Err(DifficultyError::InsufficientWindowData(s).into())
            }
    ...
    fn pruning_point_compact_headers(&self) -> Vec<(Hash, CompactHeaderData)> {
        ...
        (0..current_pp_info.index)
            ...
            .map(|hash| {
                (
                    hash,
                    self.headers_store.get_compact_header_data(hash).unwrap(),
                )
            })
            .collect_vec()
    }
...
impl ConsensusApi for Consensus {
    fn build_block_template(
        ...
        self.virtual_processor
            .build_block_template(miner_data, tx_selector, build_mode)
    }

    fn validate_and_insert_block(&self, block: Block) -> BlockValidationFutures {
        let (block_task, virtual_state_task) =
            self.validate_and_insert_block_impl(BlockTask::Ordinary { block });
        BlockValidationFutures {
            block_task: Box::pin(block_task),
            virtual_state_task: Box::pin(virtual_state_task),
        }
    }

    fn validate_and_insert_trusted_block(&self, tb: TrustedBlock) -> BlockValidationFutures {
        let (block_task, virtual_state_task) =
            self.validate_and_insert_block_impl(BlockTask::Trusted { block: tb.block });
        BlockValidationFutures {
            block_task: Box::pin(block_task),
            virtual_state_task: Box::pin(virtual_state_task),
        }
    ...
    fn populate_mempool_transaction(&self, transaction: &mut MutableTransaction) -> TxResult<()> {
        self.virtual_processor
            .populate_mempool_transaction(transaction)?;
        Ok(())
    }
    ...
    fn populate_mempool_transactions_in_parallel(
        ...
        self.virtual_processor
            .populate_mempool_transactions_in_parallel(transactions)
    }

    fn calculate_transaction_compute_mass(&self, transaction: &Transaction) -> u64 {
        self.services
            .mass_calculator
            .calc_tx_compute_mass(transaction)
    }
    ...
    fn get_virtual_merge_depth_root(&self) -> Option<Hash> {
        ...
        let root = self
            .services
            .depth_manager
            .calc_merge_depth_root(virtual_ghostdag_data, pruning_point);
        if root.is_origin() {
            None
        } else {
            Some(root)
    ...
    fn get_virtual_merge_depth_blue_work_threshold(&self) -> BlueWorkType {
        ...
        self.get_virtual_merge_depth_root()
            .map_or(BlueWorkType::ZERO, |root| {
                self.ghostdag_primary_store.get_blue_work(root).unwrap()
            })
    }
    ...
    fn estimate_block_count(&self) -> BlockCount {
        ...
        let source_score = self
            .headers_store
            .get_compact_header_data(self.get_source())
            .unwrap()
            .daa_score;
    ...
    fn is_nearly_synced(&self) -> bool {
        ...
        self.config
            .is_nearly_synced(compact.timestamp, compact.daa_score)
    }
    ...
    fn get_virtual_chain_from_block(&self, hash: Hash) -> ConsensusResult<ChainPath> {
        ...
        Ok(self
            ...
            .calculate_chain_path(hash, self.get_sink()))
    }
    ...
    fn get_chain_block_samples(&self) -> Vec<DaaScoreTimestamp> {
        ...
        if self.config.net.network_type == NetworkType::Mainnet {
            ...
            const POINTS: &[DaaScoreTimestamp] = &[
                DaaScoreTimestamp {
                    daa_score: 0,
                    timestamp: 1636298787842,
                },
                DaaScoreTimestamp {
                    daa_score: 87133,
                    timestamp: 1636386662010,
                },
                DaaScoreTimestamp {
                    daa_score: 176797,
                    timestamp: 1636473700804,
                },
                DaaScoreTimestamp {
                    daa_score: 264837,
                    timestamp: 1636560706885,
                },
                DaaScoreTimestamp {
                    daa_score: 355974,
                    timestamp: 1636650005662,
                },
                DaaScoreTimestamp {
                    daa_score: 445152,
                    timestamp: 1636737841327,
                },
                DaaScoreTimestamp {
                    daa_score: 536709,
                    timestamp: 1636828600930,
                },
                DaaScoreTimestamp {
                    daa_score: 624635,
                    timestamp: 1636912614350,
                },
                DaaScoreTimestamp {
                    daa_score: 712234,
                    timestamp: 1636999362832,
                },
                DaaScoreTimestamp {
                    daa_score: 801831,
                    timestamp: 1637088292662,
                },
                DaaScoreTimestamp {
                    daa_score: 890716,
                    timestamp: 1637174890675,
                },
                DaaScoreTimestamp {
                    daa_score: 978396,
                    timestamp: 1637260956454,
                },
                DaaScoreTimestamp {
                    daa_score: 1068387,
                    timestamp: 1637349078269,
                },
                DaaScoreTimestamp {
                    daa_score: 1139626,
                    timestamp: 1637418723538,
                },
                DaaScoreTimestamp {
                    daa_score: 1218320,
                    timestamp: 1637495941516,
                },
                DaaScoreTimestamp {
                    daa_score: 1312860,
        ...
        for header in pp_headers.iter() {
            sample_headers.push(DaaScoreTimestamp {
                daa_score: header.1.daa_score,
                timestamp: header.1.timestamp,
            });
        }
        ...
        let low_index = sc_read
            .get_by_hash(pp_headers.last().unwrap().0)
            .unwrap_option()
            .unwrap_or(high_index);
        ...
        for index in (low_index + step_size..=high_index)
            .step_by(step_size as usize)
            .chain(once(high_index))
            .dedup()
        {
            let compact = self
                .storage
                .headers_store
                .get_compact_header_data(
                    sc_read.get_by_index(index).expect("store lock is acquired"),
                )
                .unwrap();
            sample_headers.push(DaaScoreTimestamp {
                daa_score: compact.daa_score,
    ...
    fn get_virtual_utxos(
        ...
        let iter = virtual_stores
            .utxo_set
            .seek_iterator(from_outpoint, chunk_size, skip_first);
    ...
    fn apply_pruning_proof(
        ...
        self.services
            .pruning_proof_manager
            .apply_proof(proof, trusted_set)
    }

    fn import_pruning_points(&self, pruning_points: PruningPointsList) {
        self.services
            .pruning_proof_manager
            .import_pruning_points(&pruning_points)
    }
    ...
    fn import_pruning_point_utxo_set(
        ...
        self.virtual_processor
            .import_pruning_point_utxo_set(new_pruning_point, imported_utxo_multiset)
    }
    ...
    fn validate_pruning_points(&self) -> ConsensusResult<()> {
        ...
        if !self
            ...
            .is_valid_pruning_point(pp_info.pruning_point, hst)
        {
        ...
        if !self
            .services
            .pruning_point_manager
            .are_pruning_points_in_valid_chain(pp_info, hst)
        {
            return Err(ConsensusError::General(
                "past pruning points do not form a valid chain",
            ));
        }
    ...
    fn get_hashes_between(
        ...
        Ok(self
            .services
            .sync_manager
            .antipast_hashes_between(low, high, Some(max_blocks)))
    }
    ...
    fn get_header(&self, hash: Hash) -> ConsensusResult<Arc<Header>> {
        self.headers_store
            .get_header(hash)
            .unwrap_option()
            .ok_or(ConsensusError::HeaderNotFound(hash))
    }
    ...
    fn get_antipast_from_pov(
        ...
        Ok(self.services.dag_traversal_manager.antipast(
            hash,
    ...
    fn create_virtual_selected_chain_block_locator(
        ...
        Ok(self
            .services
            .sync_manager
            .create_virtual_selected_chain_block_locator(low, high)?)
    }
    ...
    fn get_block_even_if_header_only(&self, hash: Hash) -> ConsensusResult<Block> {
        let Some(status) = self
            ...
            .get(hash)
            .unwrap_option()
            .filter(|&status| status.has_block_header())
        else {
    ...
    fn get_ghostdag_data(&self, hash: Hash) -> ConsensusResult<ExternalGhostdagData> {
        ...
        let ghostdag = self
            ...
            .get_data(hash)
            .unwrap_option()
            .ok_or(ConsensusError::MissingData(hash))?;
        Ok((&*ghostdag).into())
    }
    ...
    fn get_missing_block_body_hashes(&self, high: Hash) -> ConsensusResult<Vec<Hash>> {
        ...
        Ok(self
            .services
            .sync_manager
            .get_missing_block_body_hashes(high)?)
    }
    ...
    fn get_daa_window(&self, hash: Hash) -> ConsensusResult<Vec<Hash>> {
        ...
        Ok(self
            ...
            .window_manager
            .block_window(
                &self.ghostdag_primary_store.get_data(hash).unwrap(),
    ...
    fn get_trusted_block_associated_ghostdag_data_block_hashes(
        ...
        Ok(self
            .services
            .pruning_proof_manager
            .get_ghostdag_chain_k_depth(hash))
    }
    ...
    fn create_block_locator_from_pruning_point(
        ...
        Ok(self
            .services
            .sync_manager
            .create_block_locator_from_pruning_point(high, pruning_point, Some(limit))?)
    }
    ...
    fn estimate_network_hashes_per_second(
        ...
        match start_hash {
            Some(hash) => {
                ...
                if !self
                    .headers_store
                    .has(ghostdag_data.selected_parent)
                    .unwrap()
                {
                    return Err(ConsensusError::DifficultyError(
                        DifficultyError::InsufficientWindowData(0),
                    ));
                }
            ...
            None => {
                let virtual_state = self.lkg_virtual_state.load();
                self.estimate_network_hashes_per_second_impl(
    ...
    fn are_pruning_points_violating_finality(&self, pp_list: PruningPointsList) -> bool {
        self.virtual_processor
            .are_pruning_points_violating_finality(pp_list)
    }
    ...
    fn finality_point(&self) -> Hash {
        self.virtual_processor.virtual_finality_point(
            &self.lkg_virtual_state.load().ghostdag_data,
            self.pruning_point(),
        )
    }
}
