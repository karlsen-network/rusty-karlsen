...
const fs = require('fs');
const karlsen = require('../../../../nodejs/karlsen-dev');
const {
    Wallet, setDefaultStorageFolder,
    AccountKind, Mnemonic, Resolver,
    karlsenToSompi,
    sompiToKarlsenString,
    Address
} = karlsen;
...
(async()=>{
    ...
    async function log_transactions(accountId){
        ...
        transactionsResult.transactions.forEach((tx)=>{
            ...
            list.push({
                Id: tx.id,
                Type: tx.data.type,
                Value: sompiToKarlsenString(value(tx)||0)
            });
    ...
    try {
                ...
                case "balance":
                    ...
                    Object.keys(balance).map(id=>{
                        ...
                        list.push({
                            ...
                            Mature: sompiToKarlsenString(b.mature),
                            Pending: sompiToKarlsenString(b.pending),
                            Outgoing: sompiToKarlsenString(b.outgoing),
        ...
        // Send karlsen to address
        let sendResult = await wallet.accountsSend({
            walletSecret,
            // @ts-ignore
            accountId: firstAccount.accountId,
            priorityFeeSompi: karlsenToSompi("0.001"),
            destination:[{
                // @ts-ignore
                address: firstAccount.changeAddress,
                amount: karlsenToSompi("1.567")
            }]
        });
        ...
        // Transfer karlsen between accounts
        let transferResult = await wallet.accountsTransfer({
            ...
            transferAmountSompi: karlsenToSompi("2.456"),
        });
